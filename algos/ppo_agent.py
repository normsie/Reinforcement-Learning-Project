...
class PPOAgent(BaseAgent):
    #Initialization. Most of the parameters are defined withing config 
    def __init__(self, config=None):
        super(PPOAgent, self).__init__(config)
        self.device = self.cfg.device  # ""cuda" if torch.cuda.is_available() else "cpu"
        self.policy = Policy(
            state_space = self.observation_space_dim,
            action_space = self.action_space_dim,
            env = config['env']).to(self.device)
        self.lr=self.cfg.lr
    ...
    def compute_returns(self):
        #Compute estimated returns for each time step
        returns = []
        with torch.no_grad():
    ...
    def ppo_update(self, states, actions, rewards, next_states, dones, old_log_probs, targets):
        #Implement PPO update for a minibatch
    ...
    def ppo_epoch(self):
        #Perform one epoch of PPO update
    ...
    def get_action(self, observation, evaluation=False):
        #Obtain an action from the policy given an observation
        x = torch.from_numpy(observation).float().to(self.train_device)
        action_dist, _ = self.policy.forward(x)
        if evaluation:
            #action = action_dist.probs.argmax()  #discrete action space
            action = action_dist.mean
        else:
            action = action_dist.sample()
        #aprob = action_dist.log_prob(action)
        #action = action.item() #discrete action space
        action = action.detach().cpu().numpy()
        aprob = action_dist.log_prob(torch.from_numpy(action).to(self.train_device))
        return action, aprob
    
    def store_outcome(self, state, action, next_state, reward, action_log_prob, done):
        #Store the outcome of an interaction with the environment
